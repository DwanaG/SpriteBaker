...
func populate_anim_tree(anim_list: PoolStringArray) -> void:
	...
	for a_name in anim_list:
		...
		item.add_button(Column.PLAY, preload("res://addons/model3d-to-spritesheet/icons/play.svg"), 0)
		...
		if is_loop:
			item.add_button(Column.LOOP, preload("res://addons/model3d-to-spritesheet/icons/loop_active.svg"), 0)
			item.set_metadata(Column.LOOP, true)
		else:
			item.add_button(Column.LOOP, preload("res://addons/model3d-to-spritesheet/icons/loop.svg"), 0)
...
func play_animation(item: TreeItem) -> void:
	if item.get_metadata(Column.PLAY):
		...
		item.set_button(Column.PLAY, 0, preload("res://addons/model3d-to-spritesheet/icons/stop.svg"))
		item_playing = item
...
func stop_animation(item: TreeItem, back_to_rest: bool) -> void:
	...
	item.set_metadata(Column.PLAY, false)
	item.set_button(Column.PLAY, 0, preload("res://addons/model3d-to-spritesheet/icons/play.svg"))
...
func _on_item_edited() -> void:
	...
	if col == Column.FPS:
		...
		if item.get_metadata(Column.RESTORE_FPS):
			item.add_button(Column.RESTORE_FPS, preload("res://addons/model3d-to-spritesheet/icons/clear.svg"), 0)
...
func _on_button_pressed(item: TreeItem, column: int, _id: int) -> void:
	if column == Column.PLAY:
		...
		if loop:
			item.set_button(Column.LOOP, 0, preload("res://addons/model3d-to-spritesheet/icons/loop.svg"))
		else:
			item.set_button(Column.LOOP, 0, preload("res://addons/model3d-to-spritesheet/icons/loop_active.svg"))
		loop = not loop
